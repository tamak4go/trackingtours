...
export function AuthButton() {
  ...
  if (!loaded) return <div className="w-9 h-9" />; // reserves space, avoids a layout jump once auth state resolves

  if (!user) {
    return (
      <button
        onClick={signIn}
        className="flex items-center gap-2 text-sm font-medium text-white/70 hover:text-white bg-white/[0.06] hover:bg-white/[0.1] px-4 py-2 rounded-full transition-colors"
      >
        <GoogleIcon />
        Đăng nhập Google
      </button>
